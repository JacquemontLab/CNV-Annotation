...
def generate_pdf_columns_report_duckdb(path_to_dataset, total_memory):
    ...
    if os.path.isfile(path_to_dataset) and path_to_dataset.endswith(".parquet"):
        ...
        has_subdirs = any(os.path.isdir(os.path.join(path_to_dataset, entry))
                          for entry in os.listdir(path_to_dataset))
        if has_subdirs:
            parquet_pattern = os.path.join(path_to_dataset, "**/*.parquet")
        else:
            parquet_pattern = os.path.join(path_to_dataset, "*.parquet")
    else:
        raise ValueError(f"Input path '{path_to_dataset}' is neither a .parquet file nor a directory.")
